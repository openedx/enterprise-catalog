...
from enterprise_catalog.apps.track.segment import (
...
MOCK_ANONYMOUS_USER_ID = 'anonymous_user_id'
...
@override_settings(SEGMENT_KEY=None)
@mock.patch('enterprise_catalog.apps.track.segment.logger', return_value=mock.MagicMock())
def test_track_anonymous_event_no_segment_key(mock_logger):
    track_anonymous_event(MOCK_ANONYMOUS_USER_ID, MOCK_EVENT_NAME, {})
    mock_logger.warning.assert_called_with(
        "Segment event %s for anonymous_id %s not tracked because SEGMENT_KEY not set",
        MOCK_EVENT_NAME,
...
@mock.patch('enterprise_catalog.apps.track.segment.logger', return_value=mock.MagicMock())
@mock.patch('enterprise_catalog.apps.track.segment.analytics', return_value=mock.MagicMock())
def test_track_anonymous_event_catches_exceptions(mock_analytics, mock_logger):
    mock_analytics.track.side_effect = Exception('Something went wrong')
    track_anonymous_event(MOCK_ANONYMOUS_USER_ID, MOCK_EVENT_NAME, {})
    mock_logger.exception.assert_called()
